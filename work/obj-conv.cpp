// based off of an OBJ to JS converter for MOOdle (https://obiw.ac/moodle), which itself was based off of an OBJ to IVX converter for the Louvain-li-Nux 2022 gamejam (https://github.com/obiwac/lln-gamejam-2022/blob/main/obj-to-ivx/main.cpp), which itself was based off of an old converter of mine (https://github.com/inobulles/obj-to-ivx)
...
bool load_obj(const char* path, std::vector<glm::vec3>& out_vertices, std::vector<glm::vec3>& out_normals) {
	...
	while (1) {
		...
		if (fscanf(file, "%s", line_header) == EOF) {
			...
			if (fscanf(file, "%d//%d %d//%d %d//%d\n", &vertex_index[0], &normal_index[0], &vertex_index[1], &normal_index[1], &vertex_index[2], &normal_index[2]) != 6) {
...
void index_vbo(std::vector<glm::vec3>& in_vertices, std::vector<glm::vec3>& in_normals, std::vector<uint32_t>& out_indices, std::vector<glm::vec3>& out_vertices, std::vector<glm::vec3>& out_normals) {
	std::map<packed_vertex_s, uint32_t> vertex_to_out_index;

	for (uint32_t i = 0; i < in_vertices.size(); i++) {
		packed_vertex_s packed = { .vertex = in_vertices[i], .normal = in_normals[i] };
		uint32_t index;
...
int main(int argc, char* argv[]) {
	...
	if (load_obj(argv[1], vertices, normals)) {
	...
	index_vbo(vertices, normals, indices, indexed_vertices, indexed_normals);
	...
	if (indexed_vertices.size() != indexed_normals.size()) {
		return 1;
	}

	for (uint64_t i = 0; i < indexed_vertices.size(); i++) {
		fprintf(out_fp, "%g,%g,%g,%g,%g,%g,",
